...
void Monitor::userLogin(Database* database){
        ...
        iter != database->userList.end(); iter++){
            if(iter->username == userInterface.input1){
                ...
                if(iter->password == userInterface.input2){
                    ...
                    cout << "Error password! Press ENTER to restart your login." << endl;
                    system("pause");
                    goto L1;
                }
                break;
    ...
    if(find == false){
        ...
        cin >> choice;
        if(choice){
            User* newUser = new User(userInterface.input1, userInterface.input2);
            database->addUser(newUser);
            user = newUser;
            cout << "Your account has been created. Press ENTER to user login" << endl;
            system("pause");
        } 
        goto L1;
    }
...
*/
    switch(userInterface.inputNum){
        case 1:
            {
                ...
                user->taskmanager.createTask(userInterface.input1, userInterface.input2,
        ...
        case 2:
            {
                ...
                cout << "You have successfully edited the task! Press ENTER to go back to main interface." << endl;
                system("pause");
            }
            break; 
        case 3:
            {
                ...
                cout << "You have successfully deleted the task! Press ENTER to go back to main interface." << endl;
                system("pause");
            }
            cout << "Undone" << endl;
            break;
        case 4:
            {
                ...
                userInterface.showList = user->taskmanager.filterTasks(userInterface.inputTag);
                cout << "*******************************" << endl;
                cout << "Tag: " << tag_to_string(userInterface.inputTag) << endl;
        ...
        case 5:
            {
                cout << "*******************************" << endl;
                userInterface.showList = user->taskmanager.filterPriority(URGENT);
                cout <<endl<< "Priority: " << priority_to_string(URGENT) << endl;
                userInterface.printTaskList();
                userInterface.showList = user->taskmanager.filterPriority(IMPORTANT);
                cout <<endl<<"Priority: " << priority_to_string(IMPORTANT) << endl;
                userInterface.printTaskList();
                userInterface.showList = user->taskmanager.filterPriority(OTHER_PRIORITY);
                cout <<endl<< "Priority: " << priority_to_string(OTHER_PRIORITY) << endl;
                userInterface.printTaskList();
                system("pause");
            }
            break;
